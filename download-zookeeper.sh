...
if [[ ! $(curl -s -f -I "${url}") ]]; then
    ...
    baseurl="https://archive.apache.org"
fi
...
wget -q "${baseurl}/dist/zookeeper/KEYS"

wget -q "${baseurl}/dist/zookeeper/zookeeper-${ZOOKEEPER_VERSION}/${FILENAME}.asc"
wget -q "${baseurl}/dist/zookeeper/zookeeper-${ZOOKEEPER_VERSION}/${FILENAME}.sha512"
